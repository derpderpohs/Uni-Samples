/*
Modified by Seah Hao Jian
GUID 2228156S
For Operating Systems(H) Assignment 1 part 2
*/
#include "driver.h"
...
int run_driver(CLObject* ocl, unsigned int buffer_size, int* input_buffer_1, int* input_buffer_2, int* output_buffer) {
    ...
    status_buf = clCreateBuffer(ocl->context, CL_MEM_WRITE_ONLY, sizeof (float), NULL, NULL);

    if (!input1 || !input2 || !output || !status_buf) {
        //fprintf using stderr
    ...
    if (err != CL_SUCCESS) {
        //error
		fprintf(stderr, "Error; Buffer Writing Failed %d \n", err);
		exit(EXIT_FAILURE);
    ...
    if (err != CL_SUCCESS) {
		fprintf(stderr, "Error at setting Kernel Arguments %d\n", err);
		exit(EXIT_FAILURE);
	...
	if (err != CL_SUCCESS) {
		fprintf(stderr, "Error executing kernel with error code %d\n", err);
		exit(EXIT_FAILURE);
    ...
    if (!status[0]) {
		...
		if(err != CL_SUCCESS) {
			fprintf(stderr, "Failed to read output buffer! code %d", err);
			exit(EXIT_FAILURE);
		}
    }
